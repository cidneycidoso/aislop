...
export function setup(ctx: SpindleFrontendContext) {
  ...
  const fetchCurrentText = () => {
    if (selectedChar && selectedCategory) {
      ...
      currentTextInput.update({ value: 'Loading...' })
    }
  }
  ...
  const basePromptInput = ctx.components.mountTextArea(promptSection.body, {
    value: '', rows: 3, placeholder: 'Base Prompt',
  ...
  currentTextLabel.style.cssText = 'font-weight: 500; font-size: 13px; color: var(--lumiverse-text-dim);'
  currentTextLabel.textContent = "Current Character Text:"
  ...
  const currentTextInput = ctx.components.mountTextArea(currentTextSlot, {
    value: '', rows: 5, placeholder: 'Select a character to load text...'
  })
  ...
  const resultInput = ctx.components.mountTextArea(resultSlot, {
    value: '', rows: 6, placeholder: 'AI Result will appear here. You can edit it before applying.',
  })
