...
export function setup(ctx: SpindleFrontendContext) {
  type CategoryKey = 'description' | 'personality' | 'scenario' | 'mes_example' | 'first_mes' | `alt_greeting_${number}`

  interface Character {
    id: string; name: string; description: string; personality: string
    scenario: string; first_mes: string; mes_example: string
    alternate_greetings: string[]; image_id: string | null
  }

  interface PromptConfig { base: string }
  interface VersionStore { [charId: string]: { [category: string]: string[] } }

  let selectedChar = '', selectedCategory: CategoryKey = 'description'
  let currentPrompts: PromptConfig = { base: '' }
  let fullCharList: Character[] = []
  let originalTextRaw = '', categoryVariants: string[] = []
  let selectedVersionKey = 'live', savedVersions: VersionStore = {}
  ...
  const tab = ctx.ui.registerDrawerTab({
    id: 'ai-rewriter', title: 'AI Character Rewriter', shortName: 'Rewrite',
  ...
  const catSelect = ctx.components.mountSelect(catSlot, {
    ...
    onChange: (v) => { selectedCategory = v as CategoryKey; fetchCurrentText() },
  })
  ...
  const promptSection = ctx.components.mountCollapsibleSection(promptSlot, { title: 'Edit AI Instructions', defaultExpanded: false })
  ...
  savePromptsBtn.addEventListener('click', () => { ctx.sendToBackend({ type: 'save_prompts', prompts: currentPrompts }) })
  ...
  const variantSelect = ctx.components.mountSelect(variantSelectSlot, {
    ...
    onChange: (v) => {
      ...
      if (v === 'live') { currentTextInput.update({ value: originalTextRaw }); deleteVersionBtn.style.display = 'none' }
      else { const idx = parseInt(v, 10); currentTextInput.update({ value: categoryVariants[idx] ?? '' }); deleteVersionBtn.style.display = 'block' }
    },
  })
  ...
  applyBtn.addEventListener('click', async () => {
    ...
    if (result) {
      ...
      ctx.messages.success('Applied to character card!')
    } else {
      ctx.messages.error('Failed to apply to character card.')
    }
  })
  ...
  deleteVersionBtn.addEventListener('click', () => {
    if (selectedVersionKey !== 'live' && selectedChar && selectedCategory) {
      ...
      if (savedVersions[selectedChar]?.[selectedCategory]) {
        ...
        if (savedVersions[selectedChar][selectedCategory].length === 0) delete savedVersions[selectedChar][selectedCategory]
        ctx.sendToBackend({ type: 'save_versions', versions: savedVersions })
        categoryVariants = savedVersions[selectedChar]?.[selectedCategory] ?? []
        selectedVersionKey = 'live'
        renderVariantsDropdown()
...
---`
  ...
  function getCategoryLabel(category: string): string {
    const labels: Record<string, string> = { description: 'Description', personality: 'Personality', scenario: 'Scenario', mes_example: 'Example Messages', first_mes: 'Main Greeting' }
    if (category.startsWith('alt_greeting_')) return `Alt Greeting ${parseInt(category.replace('alt_greeting_', ''), 10) + 1}`
    return labels[category] ?? category
  }

  function getCategoryText(char: Character, category: string): string {
    if (category.startsWith('alt_greeting_')) return char.alternate_greetings?.[parseInt(category.replace('alt_greeting_', ''), 10)] ?? ''
  ...
  function renderVariantsDropdown() {
    ...
    categoryVariants.forEach((_, i) => variantOptions.push({ value: i.toString(), label: `Saved Version ${i + 1}` }))
  ...
  async function loadCharacters() {
    ...
    const data = await apiGet<{ data: Character[], total: number }>('/characters?limit=200&offset=0')
    if (!data || !data.data) {
    ...
    let activeCharId = ''
    if (match && match[1] === 'characters') activeCharId = match[2]
    ...
    if (!activeCharId && fullCharList.length > 0) activeCharId = fullCharList[0].id
    ...
    charSelect.update({
      ...
      options: fullCharList.map(c => ({
        ...
        leading: c.image_id ? { type: 'image' as const, src: `/api/v1/images/${c.image_id}?size=sm` } : undefined,
      })),
    })
  ...
  ctx.onBackendMessage((payload: any) => {
    switch (payload.type) {
      case 'prompts_data': currentPrompts = payload.prompts; basePromptInput.update({ value: currentPrompts.base }); break
      case 'versions_data': savedVersions = payload.versions || {}; if (selectedChar && selectedCategory) { categoryVariants = savedVersions[selectedChar]?.[selectedCategory] ?? []; renderVariantsDropdown() }; break
      case 'generate_result': generateBtn.textContent = 'Rewrite with AI'; generateBtn.disabled = false; resultInput.update({ value: payload.result }); saveResultBtn.style.display = 'block'; break
      case 'generate_failed': generateBtn.textContent = 'Rewrite with AI'; generateBtn.disabled = false; ctx.messages.error(`Generation failed: ${payload.error}`); break
    }
  })

  const unsubTabActivate = tab.onActivate(() => { loadCharacters(); ctx.sendToBackend({ type: 'get_prompts' }); ctx.sendToBackend({ type: 'get_versions' }) })

  loadCharacters()
  ctx.sendToBackend({ type: 'get_prompts' })
  ctx.sendToBackend({ type: 'get_versions' })

  return () => { tab.destroy(); unsubTabActivate(); activeMounts.forEach(m => m?.destroy?.()) }
}
